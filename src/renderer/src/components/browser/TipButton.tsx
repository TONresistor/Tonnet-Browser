...
export function TipButton({ domain }: TipButtonProps) {
  ...
  const handleTip = useCallback(
    async (amount: string) => {
      ...
      setState('resolving')
      try {
        const result = await window.electron.wallet.resolveDomain(domain)
        const err = getIpcError(result)
        if (err) throw new Error(err)
        if (!result?.address) throw new Error('Could not resolve domain')

        setState('sending')
        const sendResult = await window.electron.wallet.send(result.address, nanoAmount)
