...
export function StartOverControl({ onReset }: { onReset: () => void }) {
  ...
  const begin = async () => {
    ...
    let result: unknown
    try {
      result = await window.electron.cocoon.getOwnerBalance()
    } catch {
  ...
  const viewPhrase = async () => {
    setError(null)
    const res = await window.electron.cocoon.walletExportMnemonic()
  ...
  const doDelete = async () => {
    setError(null)
    setPhase('deleting')
    const res = await window.electron.cocoon.walletDelete()
    const err = getIpcError(res)
    if (err) {
      setError(err)
      setPhase('idle')
