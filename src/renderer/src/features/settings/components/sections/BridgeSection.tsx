...
import { bridgeClient } from '@/features/bridge/client'
...
export const BridgeSection = memo(function BridgeSection({ onDirtyChange, sectionRef }: BridgeSectionProps) {
  ...
  const saveToMain = useCallback(async () => {
    try {
      if (policyChanged) {
        await bridgeClient.updateSettings({ defaultPolicy: draftPolicy })
        setSavedPolicy(draftPolicy)
      }
      if (configChanged && draftConfig) {
        await bridgeClient.setConfig(draftConfig)
  ...
  const loadPermissions = async () => {
    const result = await bridgeClient.getPermissions()
    if (Array.isArray(result)) setPermissions(result)
  }

  const handleRevoke = async (domain: string, scope: 'blockchain' | 'p2p' | 'write') => {
    await bridgeClient.revokePermission(domain, scope)
