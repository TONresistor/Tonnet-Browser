  ...
  const handleStake = async () => {
    ...
    try {
      ...
      if (!funded) {
        ...
        const fundResult = await cocoonClient.fundCocoon('max')
      ...
      // to the chat UI instead of resuming the wizard.
      await cocoonClient.walletMarkSetupComplete()

      setStatus('done')
      completionTimerRef.current = setTimeout(onComplete, 1500)
    } catch (error) {
      setErrorMsg((error as Error).message ?? 'Unable to start Cocoon')
      setStatus('error')
    }
  }
