...
import { SettingRow } from '@/features/settings/components/shared/SettingRow'
import { Toggle } from '@/features/settings/components/shared/Toggle'
import { RecoveryPanel } from '@/features/cocoon/components/RecoveryPanel'
import { Button } from '@/components/ui/button'
import { useOpenOrSwitchBrowserTab } from '@/features/browser/navigation'
import type { SectionProps } from '@/features/settings/components/types'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const walletInfo = await cocoonClient.walletInfo()
        if (cancelled) return
        if (!walletInfo || walletInfo.setupCompletedAt == null) {
          setContractStatus({
            label: 'Not configured',
        ...
        const stakeInfo = await cocoonClient.stakeInfo()
        if (cancelled) return
