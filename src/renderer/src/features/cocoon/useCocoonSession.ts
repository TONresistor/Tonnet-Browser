...
import type { CocoonAvailability, CocoonPendingWithdraw, CocoonState, CocoonStakeInfo } from '@shared/cocoon-types'
import { stakeBlocksRunner } from '@/features/cocoon/runner'
...
export function useCocoonSession(): UseCocoonSessionResult {
  ...
  const refresh = useCallback(() => {
    cocoonClient
      ...
      .catch((error) => {
        setWalletInfoError((error as Error).message ?? 'Failed to load wallet info')
        setWalletInfo(null)
      })
  }, [])
  ...
  useEffect(() => {
    ...
    cocoonClient
      ...
      .catch((error) => {
        if (!cancelled) setAvailabilityError((error as Error).message ?? 'Failed to load availability')
      })
    ...
    cocoonClient
      .stakeInfo()
      .then(async (stakeResult) => {
        if (cancelled) return
        const stake = stakeResult as CocoonStakeInfo | null
        setStakeInfo(stake)
        const pending = await cocoonClient.flowPending()
        if (cancelled) return
        ...
        if (terminalStake && !pendingIntent) {
          ...
          if (cancelled) return
          const ownerBalance = ownerBalanceRes.status === 'fulfilled' ? BigInt(ownerBalanceRes.value as string) : null
          const cocoonBalance =
            cocoonBalanceRes.status === 'fulfilled' ? BigInt(cocoonBalanceRes.value as string) : null
        ...
        const statusResult = await cocoonClient.status()
        if (cancelled) return
      ...
      .catch((error) => {
        if (!cancelled) {
          setStakeInfo(null)
          setStartError((error as Error).message ?? 'Failed to read Cocoon state')
        }
      })
    const offState = cocoonClient.onStateChanged((nextState) => {
      setState(nextState)
    ...
    const offWithdraw = cocoonClient.onWithdraw((e) => {
      if (e.type === 'completed') {
      ...
      cocoonClient
        .stakeInfo()
        .then((s) => {
          if (!cancelled) setStakeInfo(s as CocoonStakeInfo | null)
        })
        .catch(() => {})
    })
