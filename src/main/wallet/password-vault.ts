...
const AAD_PREFIX = 'ton-browser-wallet-v2'
const SCRYPT_N = 1 << 15
const SCRYPT_R = 8
const SCRYPT_P = 3
...
export const PasswordEnvelopeSchema = z.object({
  ...
  walletVersion: WalletVersionSchema.default('v5R1'),
  mnemonicScheme: MnemonicSchemeSchema.default('ton'),
  kdf: z.object({
...
function vaultAad(
  ...
  return Buffer.from(`${AAD_PREFIX}:${publicKey}:${walletVersion}:${mnemonicScheme}:${backupVerified ? 1 : 0}`, 'utf8')
}
...
export async function encryptWalletSecret(
  ...
  try {
    ...
    cipher.setAAD(vaultAad(publicKeyHex, walletVersion, mnemonicScheme, backupVerified))
...
export async function decryptWalletSecret(envelope: PasswordEnvelope, password: string): Promise<WalletSecret> {
  ...
  try {
    ...
    decipher.setAAD(vaultAad(parsed.publicKey, parsed.walletVersion, parsed.mnemonicScheme, parsed.backupVerified))
