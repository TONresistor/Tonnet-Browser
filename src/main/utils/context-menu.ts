 ...
 */
export function buildContextMenu(params: Electron.ContextMenuParams, options: ContextMenuOptions): void {
  ...
  if (params.isEditable) {
    ...
    menuItems.push({ label: 'Copy', accelerator: 'CmdOrCtrl+C', click: () => webContents.copy() })
  }
  ...
  if (params.linkURL) {
    ...
    if (options.onOpenLink) {
      menuItems.push({ label: 'Open Link in New Tab', click: () => options.onOpenLink!(params.linkURL) })
    }
  ...
  if (options.onNavigateBack || options.onNavigateForward || options.onReload) {
    ...
    if (options.onNavigateBack) {
      menuItems.push({
        ...
        enabled: options.canGoBack ?? false,
        click: () => options.onNavigateBack!(),
      })
    }
    if (options.onNavigateForward) {
      menuItems.push({
        ...
        enabled: options.canGoForward ?? false,
        click: () => options.onNavigateForward!(),
      })
    }
    if (options.onReload) {
      menuItems.push({ label: 'Reload', accelerator: 'CmdOrCtrl+R', click: () => options.onReload!() })
    }
  }
