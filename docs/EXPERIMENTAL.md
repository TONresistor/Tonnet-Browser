...
Room and identity aliases support `.ton` and collectible `.t.me` names.
Update clients, sequencers and replicas before using `.t.me` identity claims:
older clients can reject history containing them.

The profile prepares a wallet transaction QR or shows manual DNS setup steps.
DNS verification runs five seconds after each completed attempt, for up to ten
minutes while the linking screen remains open. Collectible `.t.me` usernames
with unsettled sales or auctions are blocked before transaction preparation.
...
Uncertain canonical operations remain pending until resolved. The Browser can
retry the exact proposal or discard local tracking with confirmation. Discarding
tracking does not cancel an operation already accepted by the room; recovery
preserves the current message draft.
